...
public class MulticasterBatchListener<T, S> implements StepExecutionListener, ChunkListener, ItemReadListener<T>,
	...
	public void register(StepListener listener) {
		if (listener instanceof StepExecutionListener executionListener) {
			this.stepListener.register(executionListener);
		}
		if (listener instanceof ChunkListener chunkListener1) {
			this.chunkListener.register(chunkListener1);
		}
	...
	private Throwable getTargetException(RuntimeException e) {
		...
		if (cause != null && cause instanceof InvocationTargetException exception) {
